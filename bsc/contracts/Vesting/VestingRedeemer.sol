...
// vest kronos, and the staking contract will stake them
...
contract VestingRedeemer is OwnableBase {
    ...
    function convertNRTToKronos(uint256 amount)
        internal
        view
        returns (uint256 kronosAmount)
    {
        kronosAmount = amount.mul(conversionNumerator).div(conversionDenominator);
    }

    function stakeDepositedTokens(uint256 _amount) internal {
        IERC20(vestingToken).approve(stakingContract, _amount);
        IKronosStaking(stakingContract).stake(_amount, address(this));
        IKronosStaking(stakingContract).claim(address(this));
    }
    ...
    function claim() public {
        ...
        //TODO check 10**9
        uint256 index = IsKronos(stakingToken).index();
        uint256 withdrawableAmount = convertNRTToKronos(
